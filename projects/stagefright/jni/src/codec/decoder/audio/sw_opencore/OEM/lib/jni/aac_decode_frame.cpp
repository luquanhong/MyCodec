...
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR  , "OPENCORE_AUDIO", __VA_ARGS__)
                            ...
                            int32* aSamplesPerFrame, Bool* aResizeFlag)
{
	...
	*/
    ...
    if (0 == iAacInitFlag)
    {
		...
		FirstBuffer = *aInBuffer;
		FirstSize = *aInBufSize;
		
        Status = PVMP4AudioDecoderConfig(&iExt, ipMem);
		
		//LOGE("Status is %d iExt.aacPlusUpsamplingFactor %d iExt.inputBufferUsedLength is %d", 
		//				Status, iExt.aacPlusUpsamplingFactor, iExt.inputBufferUsedLength);
	   ...
	   iExt.pInputBuffer         = FirstBuffer;
	   iExt.inputBufferCurrentLength = FirstSize;
    }

    iExt.inputBufferUsedLength = 0;
    Status = PVMP4AudioDecodeFrame(&iExt, ipMem);

	//LOGE("Status PVMP4AudioDecodeFrame is %d", Status);
