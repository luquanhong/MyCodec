...
void printlogToFile1(char* msg)
{
  FILE *fd=NULL;
  //LOGE("begin log=%s,len=%d",msg,strlen(msg));
  fd = fopen("/data/anr/uc_log.txt", "a");
  if(fd!=NULL)
  {
...
void *SsbSipMfcDecOpen(void)
{
	...
	if (hMFCOpen < 0)
	{
	//	printlogToFile1("SsbSipMfcDecOpen: MFC Open failure\n");
	...
	//	printlogToFile1("SsbSipMfcDecOpen: FIMV5.0 driver address mapping failed\n");
		return NULL;
	}
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecInit(void *openHandle, SSBSIP_MFC_CODEC_TYPE codec_type, int Frameleng)
{
	...
	if (openHandle == NULL)
	{
		...
		printlogToFile1("SsbSipMfcDecSetConfig: openHandle is NULL\n");
		...
		(codec_type != VC1_DEC))
	{
		...
		printlogToFile1("SsbSipMfcDecOpen: Undefined codec type.\n");
		return MFC_RET_INVALID_PARAM;
	}
	...
	if (DecArg.ret_code != MFC_RET_OK)
	{
		...
		sprintf(sz,"SsbSipMfcDecInit: IOCTL_MFC_DEC_INIT (%d) failed\n", DecArg.ret_code);
		printlogToFile1(sz);
...
void tile_to_linear_4x2_420(unsigned char *p_linear_addr, unsigned char *p_tiled_addr,unsigned char *p_tiled_caddr, unsigned int x_size, unsigned int y_size)
{
	int trans_addr;
	unsigned int i, j, k, index;
	unsigned char data8[4];
	unsigned int max_index = x_size * y_size;
  int framesize=x_size*y_size;
  int uframesize=framesize/4+framesize;
  int halfheight=y_size/2;
  int uindex=0,startindex=0;;
	// .. TILE 4x2 test
	for (i = 0; i < y_size; i = i + 16)
	{
		for (j = 0; j < x_size; j = j + 16)
		{
			...
			for (k = 0; k < 16; k++)
			{
				...
				if(i<halfheight)
					{
						 	data8[0] = p_tiled_caddr[trans_addr + 64 * k + 0];
							data8[1] = p_tiled_caddr[trans_addr + 64 * k + 1];
							data8[2] = p_tiled_caddr[trans_addr + 64 * k + 2];
							data8[3] = p_tiled_caddr[trans_addr + 64 * k + 3];
							
							uindex= (i * x_size + x_size * k + j)/2;
							p_linear_addr[framesize+uindex] = data8[0];
							p_linear_addr[uframesize+uindex] = data8[1];
							uindex++;
							p_linear_addr[framesize+uindex] = data8[2];
							p_linear_addr[uframesize+uindex] = data8[3];
							uindex++;
					}
				...
				if(i<halfheight)
					{
						 	data8[0] = p_tiled_caddr[trans_addr + 64 * k + 4];
							data8[1] = p_tiled_caddr[trans_addr + 64 * k + 5];
							data8[2] = p_tiled_caddr[trans_addr + 64 * k + 6];
							data8[3] = p_tiled_caddr[trans_addr + 64 * k + 7];
				...
				if(i<halfheight)
					{
						 	data8[0] = p_tiled_caddr[trans_addr + 64 * k + 8];
							data8[1] = p_tiled_caddr[trans_addr + 64 * k + 9];
							data8[2] = p_tiled_caddr[trans_addr + 64 * k + 10];
							data8[3] = p_tiled_caddr[trans_addr + 64 * k + 11];
				...
				if(i<halfheight)
					{
						 	data8[0] = p_tiled_caddr[trans_addr + 64 * k + 12];
							data8[1] = p_tiled_caddr[trans_addr + 64 * k + 13];
							data8[2] = p_tiled_caddr[trans_addr + 64 * k + 14];
							data8[3] = p_tiled_caddr[trans_addr + 64 * k + 15];
