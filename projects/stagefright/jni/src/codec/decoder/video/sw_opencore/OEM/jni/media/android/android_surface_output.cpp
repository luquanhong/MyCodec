...
static bool mUsingVlayer = false;
static bool mUsingGlayer = true;
static int	mTestCnt = 0;

//GEMTEK_START
static int allow_surface_control_hdmi = 0;
#define BUF_SIZE 2
...
#define V4L2_STD_480P_60_16_9		0x04000000
#define V4L2_STD_480P_60_4_3		0x05000000
#define V4L2_STD_576P_50_16_9		0x06000000
#define V4L2_STD_576P_50_4_3		0x07000000
#define V4L2_STD_720P_60		0x08000000
#define V4L2_STD_720P_50		0x09000000
#define V4L2_STD_1080P_60		0x0a000000
#define V4L2_STD_1080P_50		0x0b000000

//GEMTEK_END
//TTTT+
//to restore TTTT, just restore the lines marked by "#if 0"~"#else"
static int tv_cable=0;
//TTTT-
...
#if defined(SW_RENDERER)
    iColorConverter = NULL;
#if defined(SLSI_S5PC100)
    iColorConverterYUV = NULL;
#endif
#endif
    mInitialized = false;
    mPvPlayer = NULL;
...
#if defined(SLSI_S5PC100) || defined(SLSI_S5PC110)
    mUseHardware = false;
#endif
}
...
#if defined(SLSI_S5PC100) || defined(SLSI_S5PC110)
    /* 2009.09.07 by icarus : send all write response */
    processWriteResponseQueue(0);
#else
    if (iEosReceived) {
...
int getTvoutSetting(){
	...
	fp = fopen("/sys/tvout/output_state","r");
	if(fp==NULL) {
		;//should not happen
	}
...
int checkTvoutCable(){
	...
	fp = fopen("/sys/tvout/hpd_state","r");
	if(fp==NULL) {
		//show error, hpd_state is 0, hdmi is not to be initialized
		ret = 0;
	}
	fread(buf, sizeof(char),BUF_SIZE,fp);
	fclose(fp);
...
//GEMTEK_START
	...
	if(runonce==0){
		...
		if(fp==NULL) {
			//show error, hpd_state is 0, hdmi is not to be initialized
			allow_surface_control_hdmi = 0;
		}
		fread(buf, sizeof(char),BUF_SIZE,fp);
		fclose(fp);
		if(buf[0] == '1')
			allow_surface_control_hdmi = 1;
		else
			allow_surface_control_hdmi = 0;
		runonce = 1;
	}
...
//GEMTEK_START
    //mSurface->registerBuffers(mBufferHeap);	//Auston20100401@Register video to surfaceflinger later if HDMI is disabled.
//GEMTEK_END
...
#if defined(SLSI_S5PC100)
    iColorConverterYUV = CCYUV420SEMItoYUV420::New();
    iColorConverterYUV->Init(displayWidth, displayHeight, frameWidth, displayWidth, displayHeight, displayWidth, CCROTATE_NONE);
    iColorConverterYUV->SetMemHeight(frameHeight);
    iColorConverterYUV->SetMode(1);
#endif
#endif
    ...
    if(mNumOfPlayingContents > 2)	mNumOfPlayingContents = 2;
	mTestCnt = 0;
	
#if defined(BOARD_USES_HDMI)
#if defined(SLSI_S5PC100)
    hdmi_initialize(iVideoWidth, iVideoHeight);
    hdmi_streamoff_g0();
#endif
#if defined(SLSI_S5PC110)
//GENTEK_START
	switch(getTvoutSetting()){

	case FB_UNSET_OUTPUT_UNSET:
	case FB_WVGA_OUTPUT_LCD_WVGA:
		allow_surface_control_hdmi = 0;
		mSurface->registerBuffers(mBufferHeap);//Auston20100401@Register video to surfaceflinger if HDMI is disabled.
		break;
	case FB_720P_DISPLAY_HDMI_1080P:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		t_std_id = V4L2_STD_1080P_60;	/*0x00001000:NTSC_M, 0x0a000000:1080p*/
		HDMI_WIDTH = 1920;		//720 1920
		HDMI_HEIGHT = 1080; 	//480 1080
		allow_surface_control_hdmi = 1;
		break;
	// !!! FB_720P_DISPLAY_HDMI_1080I not supported
	case FB_720P_DISPLAY_HDMI_1080I:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		t_std_id = V4L2_STD_1080P_60;	/*0x00001000:NTSC_M, 0x0a000000:1080p*/
		HDMI_WIDTH = 1920;		//720 1920
		HDMI_HEIGHT = 1080; 	//480 1080
		allow_surface_control_hdmi = 1;
		break;
	case FB_720P_DISPLAY_HDMI_720P:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		...
		allow_surface_control_hdmi = 1;
		break;		
	case FB_720P_DISPLAY_HDMI_576P_4_3:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		t_std_id = V4L2_STD_576P_50_4_3;
		HDMI_WIDTH = 1024;
		HDMI_HEIGHT = 576;
		allow_surface_control_hdmi = 1;
		break;		
	case FB_720P_DISPLAY_HDMI_576P_16_9:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		...
		allow_surface_control_hdmi = 1;
		break;		
	case FB_720P_DISPLAY_HDMI_480P_4_3:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		t_std_id = V4L2_STD_480P_60_4_3;
		HDMI_WIDTH = 720;
		HDMI_HEIGHT = 480;
		allow_surface_control_hdmi = 1;
		break;		
	case FB_720P_DISPLAY_HDMI_480P_16_9:
		output_type = V4L2_OUTPUT_TYPE_DIGITAL;
		...
		allow_surface_control_hdmi = 1;
		break;		
	case FB_720P_DISPLAY_COMPOSITE_480I:
		output_type = V4L2_OUTPUT_TYPE_COMPOSITE;
		t_std_id = V4L2_STD_NTSC_M;
		HDMI_WIDTH = 720;
		HDMI_HEIGHT = 480;
		allow_surface_control_hdmi = 1;
		break;
  ...
  if(allow_surface_control_hdmi==1){
//GEMTEK_END    
    if(mUsingVlayer == true)
    {
    	hdmi_deinitialize();		
    }
...
//TTTT+
#if 0
	//Auston20100401@Enable g0 always.
	hdmi_gl_streamon(0);
    mUsingGlayer = true;
#endif
//TTTT-
#if 0	//Auston20100401@Remove to enable g0 always.
   	if(mUsingGlayer == true)	hdmi_gl_streamoff(0);
	...
	if(mNumOfPlayingContents > 1)
    {
		...
		LOGI("[S-Init] PlayingContents = %d, G-layer : On", mNumOfPlayingContents); 
    }
    else
    {
    	mUsingGlayer = false;
		LOGI("[S-Init] PlayingContents = %d, G-layer : Off", mNumOfPlayingContents); 
    }
#endif

//TTTT+
	if(tv_cable==1){
		//hdmi out enabled, do OSD on TV
		hdmi_gl_streamon(0);
		mUsingGlayer = true;
		;
	}else{
		//no hdmi cable, turn off HDMI
		if(mUsingGlayer == true)	hdmi_gl_streamoff(0);
...
{
#if defined(SW_RENDERER) && defined(SLSI_S5PC100)
    uint8 tempBuf[720*480*3];
#endif
#if defined(SLSI_S5PC100) || defined(SLSI_S5PC110)
    int FrameSize;
    uint8* pPhyYAddr;
...
#if defined(SW_RENDERER) && defined(SLSI_S5PC100)
    iColorConverterYUV->Convert(aData, tempBuf);
    iColorConverter->Convert(tempBuf, static_cast<uint8*>(mBufferHeap.heap->base()) + mFrameBuffers[mFrameBufferIndex]);
#endif
#if defined(SW_RENDERER) && defined(SLSI_S5PC110)
    ...
    if (mUseHardware)
    {
        ...
        memcpy(&FrameSize, aData, sizeof(FrameSize));
        memcpy(&pPhyYAddr, aData + sizeof(FrameSize), sizeof(pPhyYAddr));
        memcpy(&pPhyCAddr, aData + sizeof(FrameSize) + (AddrSize * 1), sizeof(pPhyCAddr));
        memcpy(&pVirYAddr, aData + sizeof(FrameSize) + (AddrSize * 2), sizeof(pVirYAddr));
        memcpy(&pVirCAddr, aData + sizeof(FrameSize) + (AddrSize * 3), sizeof(pVirCAddr));
#if defined(ZERO_COPY)
        memcpy(static_cast<uint8*>(mBufferHeap.heap->base()) + mFrameBuffers[mFrameBufferIndex], &pPhyYAddr, sizeof(pPhyYAddr));
        memcpy(static_cast<uint8*>(mBufferHeap.heap->base()) + mFrameBuffers[mFrameBufferIndex] + sizeof(pPhyYAddr), &pPhyCAddr, sizeof(pPhyCAddr));
#else
        memcpy(static_cast<uint8*>(mBufferHeap.heap->base()) + mFrameBuffers[mFrameBufferIndex], pVirYAddr, FrameSize);
        memcpy(static_cast<uint8*>(mBufferHeap.heap->base()) + mFrameBuffers[mFrameBufferIndex] + FrameSize, pVirCAddr, (FrameSize+1) >> 1);
#endif
    }
    else
    {
...
//GEMTEK_START
  if(allow_surface_control_hdmi==1){
//GENTEK_END
    if(mNumOfPlayingContents < 2)
    {
		if(mTestCnt == 0)	    LOGI("[S-writeFrameBuf] Video Layer On \n"); 
		mTestCnt++;
		
		hdmi_set_v_param((unsigned int)pPhyYAddr, (unsigned int)pPhyCAddr, iVideoWidth, iVideoHeight);
    	mUsingVlayer = true;
    }
    else
    {
...
#if defined(SLSI_S5PC100) || defined(SLSI_S5PC110)
    LOGV("writeFrameBuf() yaddress[0x%08x], caddress[0x%08x]\n", pPhyYAddr, pPhyCAddr);
#endif
    return PVMFSuccess;
}
...
//TTTT+
#if 0
    //mNumOfPlayingContents = 0;	//Auston20100401@Remove.
#else
	if(tv_cable==1)
		;
	else
...
#endif
//TTTT-
    LOGI("[S-Close] PlayingContents = %d", mNumOfPlayingContents); 
...
//TTTT+
#if 0
	mNumOfPlayingContents = 0;	//Auston20100401@Reset here.
#else
	if(tv_cable==1)
		mNumOfPlayingContents = 0;
	else
...
//TTTT-
    ...
    if (iColorConverter != 0)
    {
        delete iColorConverterYUV;
        iColorConverterYUV = 0;
    }
#endif
#endif
}
